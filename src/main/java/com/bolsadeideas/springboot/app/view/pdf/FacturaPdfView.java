...
public class FacturaPdfView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		cell = new PdfPCell(new Phrase(mensajes.getMessage("text.factura.form.item.nombre"), boldFont));
		tabla3.addCell(cell);
		// tabla3.addCell(mensajes.getMessage("text.factura.form.item.precio"));
		cell = new PdfPCell(new Phrase(mensajes.getMessage("text.factura.form.item.precio"), boldFont));
		tabla3.addCell(cell);

		// tabla3.addCell(mensajes.getMessage("text.factura.form.item.cantidad"));
		cell = new PdfPCell(new Phrase(mensajes.getMessage("text.factura.form.item.cantidad"), boldFont));
		tabla3.addCell(cell);

		// tabla3.addCell(mensajes.getMessage("text.factura.form.item.total"));
		cell = new PdfPCell(new Phrase(mensajes.getMessage("text.factura.form.item.total"), boldFont));
		tabla3.addCell(cell);
		...
		boldFont.setSize(14);
		cell = new PdfPCell(new Phrase(mensajes.getMessage("text.factura.form.total") + ": ",boldFont));
		cell.setColspan(3);
